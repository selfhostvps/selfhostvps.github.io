
+++
title = "本站首页"
menu = "main"
weight = 1
[params]
  no_toc = true
  no_date = true
+++

## 还不知道放什么的首页

写作计划

1. 基本概念

- 自建 VPS 的用途
- [常用 VPS 购买推荐](/post/1003-vps-list/)

2. 基本的 VPS 设置和管理

- [新购买 VPS 的初始登录和安全配置](/post/2001-new-vps-setup/)
- [常用 Linux 命令和工具](/post/2002-vps-linux-commands/)
- [购买域名、Cloudflare 配置](/post/2004-cloudflare/)
- [初始安装 1. 网站服务器 Nginx & PHP](/post/2003-install-nginx/)
- [初始安装 2. Docker 的安装、设置、基本操作](/post/2006-docker/)
- [一个基本的 Docker - Nginx 网站服务样例](/post/2005-service-deploy-example/)
- 数据库安装：[MySQL / MariaDB](/post/3306-mysql-mariadb/) 、PostgreSQL
- 数据导出和备份

3. 常用自建项目

- [如何快速自建梯子翻墙（新手版）](/post/3001-selfhost-gfw/)
- [几种自建翻墙的方案（高级版）](/post/5507-selfhost-gfw-docker-compose/)
- 自建 blog
	- [非静态 blog：Wordpress](/post/5505-wordpress/)
	- [静态 blog](/post/5510-hugo-jekyll-hexo/)：Hugo
- 常用自建项目
	- RSS 阅读器：Miniflux
	- 文章备份：Wallabag
	- 联邦宇宙 Fediverse / mastodon
    - nextcloud
    - [性能监测 uptime-kuma](/post/5501-uptime-kuma/)
    - rsshub
    - 日记 memos
    - 任务管理 vikunja
    - gitea
    - [NextChat，个人 AI 浏览器界面](/post/5503-nextchat/)
    - [adminer，从网页访问和管理 vps 数据库](/post/5506-adminer/)
    - webdav
    - [密码管理软件 Bitwarden / Vaultwarden](/post/5502-bitwarden-vaultwarden/)
    - 聊天软件 Matrix

## 大家有兴趣可以一起写啊！
